...
const MCPChatInterface: React.FC<MCPChatInterfaceProps> = ({ 
  ...
  const handleWebSocketMessage = (data: any) => {
    ...
    switch (messageType) {
      ...
      case 'chat_response_started':
      case 'chat_response_create':
        // 응답 시작 - 새 메시지 생성
        setMessages(prev => [
          ...prev,
          {
      ...
      case 'chat_response_update':
        // 전체 응답 내용으로 갱신
        updateAssistantMessage(data.message_id, data.content);
        break;
  ...
  const finalizeResponse = (data: any) => {
    ...
    // 마지막 메시지 업데이트 확인
    updateAssistantMessage(message_id, content);
  ...
  // 자동 스크롤 - 가장 마지막 메시지가 추가되거나 변경될 때만 스크롤
  const scrollToBottom = useCallback(() => {
    const options = { behavior: window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 'auto' : 'smooth' };
    ...
    try {
      ...
      wsRef.current.send({
        ...
        options: {
          temperature: 0.7,
          max_tokens: 2000,
          use_message_format: true  // 새로운 메시지 형식 사용 옵션 추가
        }
      });
        ...
        <Grid item xs={showTools ? 8 : 12}>
          ...
          >
            ...
            <Box sx={{ 
              ...
              flexDirection: 'column',
              px: 1,
              maxHeight: 'calc(100vh - 250px)',  // 채팅 영역 최대 높이 설정
              overflowY: 'scroll',              // 스크롤 항상 활성화
              scrollbarWidth: 'thin',           // Firefox 스크롤바 스타일
              '&::-webkit-scrollbar': {         // Chrome 스크롤바 스타일
                width: '8px',
              },
