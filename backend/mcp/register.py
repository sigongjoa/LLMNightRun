...
브라우저 개발자 콘솔과 윈도우 터미널 함수를 MCP 핸들러에 등록합니다.
"""
...
def register_mcp_functions(handler: MCPHandler) -> None:
    ...
    # 모든 함수 구현 가져오기
    function_impls = MCP_FUNCTION_IMPLEMENTATIONS
    ...
    for func_name, func_impl in function_impls.items():
        if func_name in function_defs:
            ...
            logger.warning(f"Function implementation found without definition: {func_name}")
