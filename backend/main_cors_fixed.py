"""
CORS 문제가 해결된 LLMNightRun API 메인 애플리케이션 모듈
기존 main.py의 복사본으로, CORS 설정을 수정하였습니다.
"""
...
# 커스텀 CORS 미들웨어 임포트
from backend.cors_middleware import CORSMiddleware as CustomCORSMiddleware
...
# CORS 설정 (기본 FastAPI CORS 미들웨어 대신 커스텀 미들웨어 사용)
app.add_middleware(
    CustomCORSMiddleware,
    allow_origins=["http://localhost:3000"],  # 프론트엔드 URL 명시적 지정
    allow_credentials=True,
    allow_methods=["GET", "POST", "PUT", "DELETE", "OPTIONS", "HEAD", "PATCH"],
    allow_headers=["Content-Type", "Authorization", "Accept", "X-Requested-With", "X-CSRF-Token"],
)

# OPTIONS 요청 전역 핸들러 등록
@app.options("/{rest_of_path:path}")
async def options_handler(request: Request):
    """
    모든 OPTIONS 요청에 대한 핸들러
    CORS preflight 요청을 수동으로 처리합니다.
    """
    return JSONResponse(
        content={},
        status_code=200
...
# GitHub 저장소 라우터 디버깅용 엔드포인트 추가
@app.get("/debug/github-repos", tags=["debugging"])
async def debug_github_repos():
    """
    GitHub 저장소 테스트용 엔드포인트
    CORS 테스트를 위한 간단한 응답을 반환합니다.
    """
    return [
        {
            "id": 1,
...
async def startup_event():
    ...
    with LogContext(event="startup"):
        logger.info(f"{settings.app_name} API 서버 시작 (CORS 수정 버전)")
...
if __name__ == "__main__":
    with LogContext(mode="standalone"):
        logger.info(f"{settings.app_name} 서버 실행 준비 (CORS 수정 버전)")
