...
class CustomMetricDefinition(Base):
    ...
    사용자가 정의한 평가 지표를 저장합니다.
    """
    __tablename__ = "ab_custom_metrics"
    
    id = Column(Integer, primary_key=True, index=True)
    name = Column(String(255), nullable=False, unique=True, index=True)
    description = Column(Text, nullable=True)
    evaluation_function = Column(Text, nullable=False)  # Python 코드 (문자열)
    parameters = Column(JSON, nullable=True)  # 추가 매개변수
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class MultiLanguageTestResult(Base):
    ...
    # 고유 제약 조건: (experiment_id, language) 조합이 유니크
    __table_args__ = (
        # UniqueConstraint('experiment_id', 'language', name='uix_multilang_experiment_language'),
    )
